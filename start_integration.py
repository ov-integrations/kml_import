...
if re.search('https', settings_data['urlOneVizion']) is None:
    url_onevizion_without_protocol = re.sub('^http://', '', settings_data['urlOneVizion'][:-1])
else:
    url_onevizion_without_protocol = re.sub('^https://', '', settings_data['urlOneVizion'][:-1])
